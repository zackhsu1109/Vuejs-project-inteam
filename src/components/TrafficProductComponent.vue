...
export default {
  props: {
    exchangeProduct: {  // 接收父組件傳遞的兌換商品方法
      type: Function,
      required: true,
    },
    isLoggedIn: {  // 接收父組件傳遞的登入狀態
      type: Boolean,
      required: true,
    },
    userPoints: {  // 接收父組件傳遞的用戶點數
      type: Number,
      required: true,
    },
  },
  ...
  methods: {
    getAllProducts() {
      fetch("http://localhost:8081/api/bonusproduct/filter?category=旅遊交通") // 獲取旅遊交通產品
